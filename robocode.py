...
class kippingMotorThread(threading.Thread):
    def __init__(self):
        self.kipping_motor = ev3.LargeMotor(ev3.OUTPUT_C)
        threading.Thread.__init__(self)
    
    def run(self):
        print("Kipping Activated!")
        while running:

            self.kipping_motor.run_forever(speed_sp=dc_clamp(kipping_speed+kipping_sidespeed))

        self.kipping_motor.stop()


kipping_motor = ev3.LargeMotor(ev3.OUTPUT_C)
...
for event in gamepad.read_loop():   
    if event.type == 3:
        ...
        if clamp_speed < 100 and forward_speed > -100:
            clamp_speed = 0
        if event.code == 5:
            kipping_speed = -scale_stick(event.value)
    if event.type == 1:
        if event.code == 304 and event.value == 1:
            kipping_motor.run_timed(time_sp=100, speed_sp=500)
